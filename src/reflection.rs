...
pub unsafe fn find_methods_in_assembly_by_param(
    ...
    log!(
        "Scanning {} classes in assembly for methods taking '{}'...",
        class_count,
        param_type_name
    );
    ...
    for i in 0..class_count {
        ...
        if let Some(method) = find_method_taking_param_by_name(klass, param_type_name) {
            ...
            let method_name = CStr::from_ptr(method_name_ptr)
                .to_string_lossy()
                .to_string();
    ...
    if results.is_empty() {
        log!(
            "No methods found taking parameter type '{}'",
            param_type_name
        );
    } else {
...
unsafe fn resolve_enum_to_string(enum_ptr: *mut u8, klass: *mut RawIl2CppClass) -> Value {
        ...
        if !cache_map.contains_key(&class_name) {
            ...
            loop {
                ...
                if (flags & FIELD_ATTRIBUTE_STATIC) != 0 && (flags & FIELD_ATTRIBUTE_LITERAL) != 0 {
                    ...
                    FN_FIELD_STATIC_GET_VALUE.unwrap()(
                        field,
                        &mut static_val_buf as *mut i32 as *mut c_void,
                    );
...
unsafe fn read_value_from_addr(
    ...
    match type_enum {
        ...
        0xC => Number::from_f64(*(addr as *const f32) as f64)
            .map(Value::Number)
            .unwrap_or(Value::Null),
        0xD => Number::from_f64(*(addr as *const f64))
            .map(Value::Number)
            .unwrap_or(Value::Null),
        0xE => {
            ...
            if !ftype.is_null() {
                ...
                if !klass.is_null() {
                    ...
                    if name.starts_with("Obscured") {
                        if let Some(val) = try_decrypt_obscured(addr as *mut c_void, klass, &name) {
                            return val;
                        }
                    }
                    ...
                    return convert_struct_to_value(
                        addr as *mut c_void,
                        klass,
                        depth + 1,
                        visited,
...
pub unsafe fn convert_object_to_value(
        ...
        if !element_class.is_null() {
            ...
            if !is_value_type {
                ...
                for i in 0..len {
                    ...
                    if !elem_ptr.is_null() {
                        vec.push(convert_object_to_value(
                            elem_ptr,
                            depth + 1,
                            visited,
                            sensitive_fields,
                ...
                if stride > 0 {
                    ...
                    for i in 0..len {
                        ...
                        let val = if FN_CLASS_IS_ENUM.unwrap()(element_class) {
                            ...
                            let name = CStr::from_ptr(name_ptr).to_string_lossy();
                            if name.starts_with("Obscured") {
                                if let Some(v) = try_decrypt_obscured(
                ...
                return Value::String(format!(
                    "<Array: {} (Struct Array, Unknown Stride)>",
                    class_name
                ));
            }
        }
    ...
    while !current_dump_class.is_null() {
        ...
        loop {
            ...
            if is_field_blacklisted(&name, sensitive_fields) {
                continue;
            }
